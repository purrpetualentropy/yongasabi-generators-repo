function argsArrayFormatter(argsArray, tensesArray, verbType) {
    // argsArrayFormatter VERSION: 1.0.0
    ...
    if (forceArgs == true)
    {
        ...
        for (let i = 0; i < forcedArgsArray.length; i++) {
            ...
            miniDict = miniDict.filter((obj) => obj.args[forcedArgsArray[i]] === true || obj.args[forcedArgsArray[i]].includes(-1) === false);
            console.log(`LOG: miniDict now:`);
            console.dir(miniDict);
        }
    }
    ...
    if (forceArgs == true) {
        ...
        } else
        {
          ...
          if (angsaIndex == undefined) {console.log(`LOG: angsa was never present anyway`)} else
            if (angsaIndex != -1 && angsaIndex != undefined) {console.log(`LOG: angsaIndex is ${angsaIndex}, destroying`);
                                         ...
                                         // this doesn't work but it's getting late and i need to go to bed
                                         // so im just gonna put up a bug report and forget about it
        verbIndex = Math.floor(Math.random() * (miniDict.length - 1 + 1));
        verb = miniDict[verbIndex]
        formattedArgsArray[0] = verb;
        ...
        // this .............................. doesn't work
        // we'll need to remove angsa from the array entirely if loc and dirobj are used.
        // actually if we're forcing any args more than loc or dirobj.
    }
