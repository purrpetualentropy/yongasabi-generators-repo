function tenseSuffixer(verb, verbLength, tensesArray) {
    // tenseSuffixer VERSION: 1.0.0
    let cutVerb = verb.slice(0, verb.length - 1);
    let dakaVerb = (verb.slice( verb.length - 4) == "daka" && verb.length != 4);
    ...
    console.log(`LOG: Found word ${verb} and root length ${verbLength}! cutVerb is ${cutVerb}! dakaVerb is ${dakaVerb}!`);

    if (dakaVerb) {
        console.log(`LOG: dakaVerb detected!`);
        cutVerb = "dak";
        console.log(`LOG: cutVerb now ${cutVerb}, proceed to suffixing`);
        for (let i = 0; i < tensesArray.length; i++) {
            ...
            switch (currSuffix) {
                ...
                case "neg":
                    ...
                    if (prevSuffix == "plain") {console.log(`LOG: prevSuffix is plain, post-fixing!`); suffixedVerb += " yong"; console.log(`LOG: verb ${verb} suffixed with neg plain into ${suffixedVerb}!`); prevSuffix = "neg"} else
    ...
    if (tensesArray.includes("present") && tensesArray.includes("habit")) {
        console.log(`LOG: emphabit, do not contract`);
        console.groupEnd();
        suffixedVerb = verb.slice(0, verb.length - 4) + suffixedVerb
        return suffixedVerb;
    } else
    {console.log(`LOG: verb itself is not daka, suffix`);
    suffixedVerb = verb.slice(0, verb.length - 4) + suffixedVerb.slice(2)
