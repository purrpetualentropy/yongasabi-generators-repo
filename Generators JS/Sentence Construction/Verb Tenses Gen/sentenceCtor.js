function sentenceCtor (verbType, tensesArray, argsArray, preVAlters, postVAlters) {
    ...
    console.groupCollapsed(wordDict);
    console.dir(wordDict);
    console.groupEnd();
    ...
    for (let i = 1; i < argsArray.length; i++) {
        ...
        // also add the word to the used args list and increase the index at which we write into used args
         try {if (argsDict[targetIndex].sentenceTone != undefined) {sentenceTone = argsDict[targetIndex].sentenceTone}} catch (error) { 	console.error(`couldn't define sentenceTone due to: ${error}`)}
    ...
    if (altersDict.length != 0) {
        ...
        switch (preVAlters[i]) {
            ...
            case "compound":
                ...
                if (compoundType.toString().startsWith("3")) {
                    console.log(`LOG: compoundType startsWith 3 detected, compoundType is ${compoundType}`);
                    // check if we have a head, dependent, or counter

                    if (compoundPosition == compoundType + "-head") {
                        console.log(`LOG: head detected. retrieving dependent and counter`);
                        argsDict = wordDict.filter((obj) => obj.compoundTypes != undefined && obj.compoundTypes.includes(compoundType) && usedArgs.includes(obj.word) == false && obj.compoundPart.includes(compoundType + "-dependent"));
                        console.dir(argsDict);
                        alterIndex = Math.floor(Math.random() * (argsDict.length - 1 + 1));
                        let dependent = argsDict[alterIndex];
                        argsDict = wordDict.filter((obj) => obj.compoundTypes != undefined && obj.compoundTypes.includes(compoundType) && usedArgs.includes(obj.word) == false && obj.compoundPart.includes(compoundType + "-counter"));
                        console.dir(argsDict);
                        alterIndex = Math.floor(Math.random() * (argsDict.length - 1 + 1));
                        let counter = argsDict[alterIndex];
                        try {
                            console.log(`LOG: dependent now ${dependent.word}, counter now ${counter.word}`);
                            console.log(`LOG: now splicing`);
                            targetIndex = sentenceArray.findIndex(object => {
                                return object.word == altersDict[targetIndex].word && !object.legalAlters.includes("none")
                            });
                            sentenceArray.splice(targetIndex, 0, {word: counter.word, arg: "none", legalAlters: ["none"], isCompound: true, compoundPart: "counter"})
                            sentenceArray.splice(targetIndex, 0, {word: dependent.word, arg: "none", legalAlters: ["none"], isCompound: true, compoundPart: "dependent"})
                            sentenceArray[targetIndex].legalAlters = ["none"];
                            console.log(`sentenceArray now:`);
                            console.dir(sentenceArray)
                            if (argsDict[alterIndex].sentenceTone != undefined) {sentenceTone = argsDict[alterIndex].sentenceTone}
                        } catch (error) {
                            console.error(`ERROR: after assigning dependent and counter, did NOT continue writing with error:`);
                            console.error(error);
                        }
                    ...
                    // head, measure:
                    // find dependent and counter, assign
                    // write word at head index
                    // then write count at head index

                    if (compoundPosition == compoundType + "-dependent") {
                        console.log(`LOG: dependent detected. retrieving head and counter`);
                        argsDict = wordDict.filter((obj) => obj.compoundTypes != undefined && obj.compoundTypes.includes(compoundType) && usedArgs.includes(obj.word) == false && obj.compoundPart.includes(compoundType + "-head"));
                        console.dir(argsDict);
                        alterIndex = Math.floor(Math.random() * (argsDict.length - 1 + 1));
                        let head;
                        if ((Math.floor(Math.random() * (2 - 1 + 1)) + 1) == 2) {head = argsDict[alterIndex]} else {head = wordDict.filter((obj) => obj.word == altersDict[targetIndex].word); head = {word: head[0].measureWord}}
                        if (head == undefined) {
                            head = wordDict.filter((obj) => obj.word == altersDict[targetIndex].word);
                            head = {word: head[0].measureWord}
                        }
                        argsDict = wordDict.filter((obj) => obj.compoundTypes != undefined && obj.compoundTypes.includes(compoundType) && usedArgs.includes(obj.word) == false && obj.compoundPart.includes(compoundType + "-counter"));
                        console.dir(argsDict);
                        alterIndex = Math.floor(Math.random() * (argsDict.length - 1 + 1));
                        let counter = argsDict[alterIndex];
                        try {
                            console.log(`LOG: head now ${head.word}, counter now ${counter.word}`);
                            console.log(`LOG: now splicing`);
                            targetIndex = sentenceArray.findIndex(object => {
                                return object.word == altersDict[targetIndex].word && !object.legalAlters.includes("none")
                            });
                            sentenceArray.splice(targetIndex + 1, 0, {word: head.word, arg: "none", legalAlters: ["none"], isCompound: true, compoundPart: "head"})
                            sentenceArray.splice(targetIndex + 1, 0, {word: counter.word, arg: "none", legalAlters: ["none"], isCompound: true, compoundPart: "counter"})
                            sentenceArray[targetIndex].legalAlters = ["none"];
                            console.log(`sentenceArray now:`);
                            console.dir(sentenceArray)
                            if (argsDict[alterIndex].sentenceTone != undefined) {sentenceTone = argsDict[alterIndex].sentenceTone}
                    ...
                    // dependent, item:
                    // find counter and measure, assign
                    // when finding measure, keep in mind that we should flip a coin
                    // on whether to use the dependent's measureWord property, or a random measure word
                    // also, if  there are no measures, just use the measureWord
                    // write measure at +1 index
                    // then write counter at +1 index, which should write them properly

                    if (compoundPosition == compoundType + "-counter") {
                        console.log(`LOG: counter detected. retrieving head and dependent`);
                        argsDict = wordDict.filter((obj) => obj.compoundTypes != undefined && obj.compoundTypes.includes(compoundType) && usedArgs.includes(obj.word) == false && obj.compoundPart.includes(compoundType + "-head"));
                        console.dir(argsDict);
                        alterIndex = Math.floor(Math.random() * (argsDict.length - 1 + 1));
                        let head;
                        if ((Math.floor(Math.random() * (2 - 1 + 1)) + 1) == 2) {head = argsDict[alterIndex]} else {head = wordDict.filter((obj) => obj.word == altersDict[targetIndex].word); head = {word: head[0].measureWord}}
                        if (head == undefined) {
                            head = wordDict.filter((obj) => obj.word == altersDict[targetIndex].word); head = {word: head[0].measureWord}
                        }
                        argsDict = wordDict.filter((obj) => obj.compoundTypes != undefined && obj.compoundTypes.includes(compoundType) && usedArgs.includes(obj.word) == false && obj.compoundPart.includes(compoundType + "-dependent"));
                        console.dir(argsDict);
                        alterIndex = Math.floor(Math.random() * (argsDict.length - 1 + 1));
                        ...
                        try {
                            console.log(`LOG: head now ${head.word}, dependent now ${dependent.word}`);
                            console.log(`LOG: now splicing`);
                            targetIndex = sentenceArray.findIndex(object => {
                                return object.word == altersDict[targetIndex].word && !object.legalAlters.includes("none")
                            });
                    ...
                    // counter (special type only for this)
                    // find word and measure
                    // again, flip coin whether to use the word's measureWord property or a random measure word
                    // and ditto above with the nullchecking
                    // write measure at +1 index
                    // then word at counter index

                    // after all that, make sure our target word cannot be affected by other alters
                    // im NOT allowing negated MWCs.
                } else
                if (compoundPosition == compoundType + "-head") {
    ...
    console.log(`LOG: took ${endTime - startTime} ms`);
    console.groupEnd();
